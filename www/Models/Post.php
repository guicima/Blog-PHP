...
class Post extends Database
{
    ...
    public function save() : void
    {
        ...
        if ($this->id != null) {
            self::query(
                'UPDATE articles SET title = :title, tiny_text = :tiny_text, text = :text, state = :state, modified_at = :modified_at WHERE id = :post_id', 
                array(
                    ':title' => $this->title, 
                    ':tiny_text' => $this->tiny_text, 
                    ':text' => $this->text, 
                    ':state' => $this->state, 
                    ':post_id' => $this->id, 
            ...
            self::query(
                'INSERT INTO articles VALUE (null, :title, :tiny_text, :text, null, :state, :modified_at, :created_at)', 
                array(
                    ':title' => $this->title, 
                    ':tiny_text' => $this->tiny_text, 
                    ':text' => $this->text, 
    ...
    public function delete() : void
    {
        if ($this->state == 'TRASH') {
            self::query(
                'DELETE FROM articles WHERE id=:postid', 
                array(
                    ':postid' => $this->id
                )
            );
    ...
    {
        $comments = self::query(
            'SELECT id FROM comments WHERE article_id=:article_id AND is_response = FALSE AND state=:state ORDER BY created_at DESC', 
            array(
                ':article_id' => $this->id, 
                'state' => 'PUBLIC'
            )
